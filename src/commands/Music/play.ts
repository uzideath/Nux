...
export default new Command({
	...
	async commandRun(interaction) {
		...
		try {
			...
			const botPermissions = userVoiceChannel.permissionsFor(me!);
			if (!botPermissions?.has(PermissionFlagsBits.Connect) || !botPermissions?.has(PermissionFlagsBits.Speak)) {
				return interaction.editReply({
					content: 'I do not have the necessary permissions to join and play in your voice channel.',
				});
			}
			...
			if (player) {
				if (player.voiceChannel) {
					if (player.voiceChannel !== userVoiceChannel.id) {
						return interaction.editReply({
	...
	async messageRun(message) {
		try {
			...
			if (res.loadType === 'playlist') {
				...
				message.channel.send(`Playlist \`${res.playlistInfo.name}\` loaded with ${res.tracks.length} tracks.`);
			} else {
				const track = res.tracks[0];
				track.info.requester = message.author;
				player.queue.add(track);
